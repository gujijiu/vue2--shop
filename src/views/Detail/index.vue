    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="goodsInfo.skuImageList || [{}]" />
          <!-- 小图列表 -->
          <ImageList :skuImageList="goodsInfo.skuImageList || [{}]"  />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          <div class="goodsDetail">
            <h3 class="InfoName">{{goodsInfo.skuName}}</h3>
            <p class="news">{{goodsInfo.skuDesc}}</p>
            <div class="priceArea">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" v-for="(spuSale,index) in spuSaleAttr.spuSaleAttrValueList" :key="spuSale.id" :class="{active:spuSale.isChecked=='1'}" @click="changeActive(spuSaleAttr.spuSaleAttrValueList,spuSale)" >{{spuSale.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
<script>
  ...
  export default {
    ...
    mounted(){
      //派发action获取产品详情
      this.$store.dispatch('detail/goodsInfo',this.$route.params.goodsId)
    },
    computed:{
      ...mapGetters({
    ...
    methods: {
      changeActive(spuSaleAttrValueList,spuSale){
      ...
      async addShopCart(){
        //1、派发action 发送请求 将产品添加到服务器
        //2、服务器存储成功 进行路由跳转
        //3、失败给用户提示
        try{
          await this.$store.dispatch('detail/addOrUpdateShouCart',{goodsId:this.$route.params.goodsId,goodsNum:this.skuNum});
