    ...
    <div class="pay-container">
      <div class="checkout-tit">
        <h4 class="tit-txt">
          ...
          <!-- <el-button type="primary">1223</el-button> -->
        </h4>
        <div class="paymark">
...
import QRCode from "qrcode";
  export default {
    ...
    //路由配置不能获取组件实例`this`--因为当守卫执行前 组件实例还没创建
    // beforeRouteEnter(to, from, next){
    // },
    // beforeRouteUpdate(to, from, next){
    //   //在当前路由改变，但是该组件被复用时调用
    //   //举例来说，对于一个带有动参数的路径/fo0/:id,在/foo/1和/foo/2之间跳转时候，
    //   //由于会渲染同样的F00组件，因此组件实例会被复用。而这个钩子就会在这个情况下被调用。
    //   //可以访问组件实例`this`
    // },
    ...
    mounted(){
      //工作项目 尽量别在生命周期/钩子中使用async|await
      this.getPayInfo();
    },
    methods: {
      //获取支付信息
      async getPayInfo() {
        let result = await this.$http.reqGetPayInfo(this.$route.query.orderId);
        if (result.code == 200) {
      ...
      async open(){
        ...
        this.$alert(`<img src=${url}>`, '请微信扫描二维码支付', {
